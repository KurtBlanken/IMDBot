...
def handle_pref(data):
  ...
  for pref in data['pos']:
    pospref=pref
    if pospref[0]=='actor':     
      data['output'] = random.sample(['Yeah ' + str(imdbi.get_person(pospref[1])) + ' is ' + str(data['prefs'][pospref[0], pospref[1]]),
      'I agree', 'Ok'], 1)
...
def handle_trivia(data):
  ...
  if data['trivia']['attr'] == 'when':
    data['output'] = random.sample(['The production year was ' + str(data['trivia']['answer']), str(data['trivia']['answer'])], 1)
